...
class game_variable: # Game variables
    def __init__(self):
        ...
        self.splitConfirmation2 = False
        self.splitOverride1 = False
        self.splitOverride2 = False


        self.DoubleDownActivation = [0, 0, 0, 0]
        self.DoubleDownChipValueTempList = [0, 0, 0, 0]

        self.DoubleDownChipTempList1 = []
        self.DoubleDownChipTempList2 = []
        self.DoubleDownChipTempList3 = []
        self.DoubleDownChipTempList4 = []
    ...
    def chip_object(self):
        ...
        for index_var in GV.chipDisplayPriority:
            ...
            if GV.mousePosChange and index_var == GV.chipDisplayPriority[-1]:
                if index_var in GV.chipBet1 or index_var in GV.chipBet2 or index_var in GV.chipBet3 or index_var in GV.chipBet4 or index_var in GV.chipExchange:
                ...
                chipOutlineWidth = 2
            elif index_var in GV.tempChipBet1 or index_var in GV.tempChipBet4:
                chipOutlineColour = GV.bright_green
                chipOutlineWidth = 2
        ...
        if GV.bettingGame:
            if 1 in GV.gamefocus:
                for index, value in enumerate(GV.gamefocus):
                    if value == 1:
                        if GV.DoubleDownActivation[index] == 1 and len(GV.CardHands[index]) == 2:
...
class game_functions:
                    ...
                    if GV.BET_HIT_Button and GV.bettingGame is False:
                        ...
                        if GV.gamefocus[1] == 1:
                            if GV.splitActivation1:
                                ...
                                for indexb, value in enumerate(GV.gameCHIPS[0]):
                                    if value != 0:
                                        CHIPS[indexb] -= value
                                GV.DoubleDownChipTempList[0].clear()
                                GV.DoubleDownChipTempList[1].clear()
                            ...
                            if GV.splitActivation2:
                                ...
                                GV.tempChipBetValues[0] = 0
                                GV.tempChipBetValues[1] = 0
                                for indexb, value in enumerate(GV.gameCHIPS[3]):
                                    if value != 0:
                                        CHIPS[indexb] -= value
                                GV.DoubleDownChipTempList[2].clear()
                                GV.DoubleDownChipTempList[3].clear()
    ...
    def betting_area(self):
        for self.indexChipPosition in reversed(GV.chipDisplayPriority):
            ...
            bet3_remove = True
            bet4_remove = True
            for indexs, o in enumerate(GV.DoubleDownChipTempListremove):
                GV.DoubleDownChipTempListremove[indexs] = 1
            ...
            if -75 <= rectRotatedx1 <= 75 and -100 <= rectRotatedy1 <= 100:
                bet2_remove = False
                bet1_remove = False
                GV.DoubleDownChipTempListremove[1] = 0
                            ...
                            if GV.DoubleDownChipTempList:
                                for indexes, list_var in enumerate(GV.DoubleDownChipTempList[2:4]):
                                    ...
                                    if list_var:
                                        ...
                                        for values in list_var:
                                            GV.DoubleDownChipValueTempList[indexes] += int(GV.chipValues[values[0]])
                if len(GV.CardHand3) == 2:
                    if len(GV.CardHand3[0]) == 3 and len((GV.CardHand3[1])) == 3:
                        if GV.Values[int((GV.CardHand3[0])[1:3]) -2] == GV.Values[int((GV.CardHand3[1])[1:3]) -2]:
                        ...
                        if GV.Values[int((GV.CardHand3[0])[1]) -2] == GV.Values[int((GV.CardHand3[1])[1]) -2]:
                            if self.indexChipPosition not in GV.tempChipBet4 and self.indexChipPosition not in GV.chipBet3 and not GV.splitOverride2 and GV.bettingGame:
                                GV.tempChipBet4.append(self.indexChipPosition)
        ...
        for index, hand in enumerate(GV.gamefocus):
            if hand != 0:
                if GV.gameStart[index] == 1 and GV.addCard[index] == 1:
                    ...
                    GV.CardValues[2].clear()
                    GV.CardValues[3].clear()
                    ...
                    if len((GV.CardHands[2])[-1]) == 3:
                        GV.CardValues[3].append(int(GV.Values[int((GV.cardDeck[0])[1:3])-2]))
                    else:    
                    ...
                    (GV.cardStartPos[3])[1] -= 20

                    #---
                    GV.CardHands[2].pop(-1)
                    GV.cardPositions[2].pop(-1)
                    (GV.cardStartPos[2])[0] -= 20
                    (GV.cardStartPos[2])[1] += 20
                    GV.CardValues[2].pop(-1)

                    
                    # first hand card replce ---
                    GV.CardHands[3].append(GV.cardDeck[0])
                    GV.cardDeck.append(GV.cardDeck[0])
                    ...
                    # second hand card add ---
                    GV.CardHands[2].append(GV.cardDeck[2])
                    GV.cardDeck.append(GV.cardDeck[2])

                    if len(GV.cardDeck[2]) == 3:
                        GV.CardValues[2].append(int(GV.Values[int((GV.cardDeck[0])[1:3])-2]))
                    else:    
                    ...
                    GV.splitConfirmation2 = False
                    GV.splitOverride2 = True

                    for chip in GV.chipBet4:
                        GV.gameChipPos4.append(GV.chipPositions[chip[0]][chip[1]])
                        GV.gameBet[3] += int(GV.chipValues[chip[0]])
                    ...
                    for indexs, value in enumerate(GV.CardValues):
                        GV.CardValues[indexs].clear()
        ...
        if GV.dTurn:
            ...
            if 4 in GV.HandState and 21 >= GV.dHandValue and len(GV.dCardHand) == 5:
                ...
                for indexinges, i in enumerate(GV.HandState):
                    ...
                    elif GV.HandValues[indexinges] == GV.dHandValue:
                        GV.gameOutput[indexinges] = 1
                    else:
                        GV.gameOutput[indexinges] = 2
